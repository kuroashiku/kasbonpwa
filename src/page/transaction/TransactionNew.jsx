...
import { getTransaction, deleteTransaction, getCredit, saveCredit, getCountTransaction } from "../../api/Transaction";
...
import POSSuccessJoinBill from "../pos/POSSuccessJoinBill";
...
export default function TransactionNew() {
  const { setMenuOpen, filters, setFilters, lang, cookies, rowsPerPage, currency, setItemsCheckout,
    setNotaItemsCheckout, setMoney, setTotalPay, setNotaItemsCheckoutSplitBill, setModeCil, setItemsCheckoutBill} = useContext(AppContext);
  ...
  const initData = useCallback(() => {
    ...
    _newfilter.forEach((_item, index) => {
      ...
      setLoading(true);
      // setPage(1);
      const { data, error } = await getCountTransaction({
        lok_id: cookies.lok_id,
        page: page,
        rows: rowsPerPage,
        loaditems: "yes",
      ...
      setTotalnota(data.total_trans);
      setTotalitem(data.total_transitem);
      setTotalnominal(parseInt(data.total_nota));
    };
    initcount();
        ...
        data.map((i, index) => {
          i.notaitems.map((ii, indexi) => {
            if (cookies.lok_type == "laundry") {
              ii.service_level_satuan0 = ii.nit_service_level_satuan0?JSON.parse(ii.nit_service_level_satuan0):[];
            }
          });
        });
    ...
    useEffect(() => {
      ...
      if (transactions[0] && transactions[0].not_id && !filters[0]) {
        const init = async () => {
          const { data, error } = await getTransaction({
            lok_id: cookies.lok_id,
            page: page,
    ...
    const init = async () => {
      ...
      if (data[0].cil_id) {
        ...
        setCredits({
          ...
          cil_cicilan: 0,
          cil_sisa: Math.abs(item.kembalian),
          cil_carabayar: "KAS",
        });
      }
  ...
  const saveData = useCallback(async () => {
    ...
    credits.not_id = transById.not_id;
  ...
  }, [transById, credits,keyword, filters, page]);
  ...
  function handleJoinPrint() {
    const trans = cloneDeep(transactions);
    let arr=[];
    transCheckId.map((ii, indexi) => {
      let _newfilter = trans.filter(function (object) {
        return object.not_id === ii;
      });
      arr.push(_newfilter[0]);
    });
    setItemsCheckoutBill(arr);
    setSuccessJoin(true);
  }
function handlePrintSplitBill(item) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    setPage(1)
    initData();
  }, [transById,transCheckId,keyword, filters, page]);
  ...
  if (successJoin) {
    return <POSSuccessJoinBill exit={() => setSuccessJoin(false)} />;
  }
