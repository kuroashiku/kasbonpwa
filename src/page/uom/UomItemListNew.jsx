...
export default function UomItemListNew() {
...
const handleSelect = (item = ItemListModel()) => {
  ...
  function handleOpenPrint() {
    const init = async () => {
      ...
      if (error) {
        ...
        console.log(convertItemListToCheckout(data))
  ...
  const handleSetImage = (url,id) => {
    setItemById({
      ...itemById,
      ["itm_urlimage"+(id+1)]: url,
    });
  };
                        ...
                        <Tabs value={activeTab}>
                            ...
                            <TabsBody>
                                ...
                                <TabPanel key={value} value={value}>
                                    <div className="mb-6">
                                        {id==0?<ImageUpload
                                          image={itemById['itm_urlimage'+(id+1)]}
                                          id={"upl-image"+(id+1)}
                                          disabled={true}
                                        />:<ImageUpload
                                          image={itemById['itm_urlimage'+(id+1)]}
                                          onSuccess={(url) => handleSetImage(url,id)}
                                          onRemove={() => handleSetImage(null,id)}
                                          id={"upl-image"+(id+1)}
                                          disabled={false}
                                        />}
                                    ...
                                    <div className="mb-6">
                                        {id==0?<InputSimple value={itemById['itm_satuan'+(id+1)]}
                                            label={dictionary.stock.uom.unitName[lang]}
                                            disabled
                                        />:<InputSimple value={itemById['itm_satuan'+(id+1)]}
                                            label={dictionary.stock.uom.unitName[lang]}
                                            name={`itm_satuan${id+1}`}
                                            onChange={(evt) => handleChange(evt, 'itm_satuan'+(id+1))}
                                            //error={errors['itm_satuan'+(id+1)]}
                                        />}
                                    </div>
                                    <div className="mb-10">
                                        {id==0?<InputMoney value={1}
                                            label={dictionary.stock.uom.coef[lang]}
                                            disabled
                                        />:<InputMoney value={itemById['itm_satuan'+(id+1)+'of1']?itemById['itm_satuan'+(id+1)+'of1']:0}
                                            label={dictionary.stock.uom.coef[lang]}
                                            name={`itm_satuan${id+1}of1`}
                                            onChange={(evt) => handleChange(evt, 'itm_satuan'+(id+1)+'of1')}
                                            //error={errors['itm_satuan'+(id+1)+'of1']}
                                        />}
                                    </div>
                                    <div className="mb-10">
                                        {id==0?<InputMoney value={itemById['itm_satuan'+(id+1)+'hpp']}
                                            currency={currency}
                                            label={dictionary.stock.uom.hpp[lang]}
                                            disabled
                                        />:<InputMoney value={itemById['itm_satuan'+(id+1)+'hpp']?itemById['itm_satuan'+(id+1)+'hpp']:0}
                                            currency={currency}
                                            label={dictionary.stock.uom.hpp[lang]}
                                            name={`itm_satuan${id+1}hpp`}
                                            onChange={(evt) => handleChange(evt, 'itm_satuan'+(id+1)+'hpp')}
                                            //error={errors['itm_satuan'+(id+1)+'hpp']}
                                        />}
                                    </div>
                                    <div className="mb-10">
                                        {id==0?<InputMoney value={itemById['itm_satuan'+(id+1)+'hrg']}
                                            currency={currency}
                                            label={dictionary.stock.uom.price[lang]}
                                            disabled
                                        />:<InputMoney value={itemById['itm_satuan'+(id+1)+'hrg']?itemById['itm_satuan'+(id+1)+'hrg']:0}
                                            currency={currency}
                                            label={dictionary.stock.uom.price[lang]}
                                            name={`itm_satuan${id+1}hrg`}
                                            onChange={(evt) => handleChange(evt, 'itm_satuan'+(id+1)+'hrg')}
            ...
            >
              <div className="flex items-center">
                ...
                <div className="mx-2 flex-grow">
                  <SearchNavbar onSearch={handleFilter} value={keyword} label={dictionary.search.convertion[lang]} />
                </div>
