import { Button, Dialog, DialogHeader, DialogBody, DialogFooter, IconButton, List, ListItem, ListItemPrefix, 
	ListItemSuffix, Navbar, Option, Switch, Select, Typography} from "@material-tailwind/react";
import { useCallback, useContext, useState, useRef, useEffect } from "react";
import { ArrowRightIcon, ChevronLeftIcon, CubeIcon, MinusCircleIcon,
	ArchiveBoxIcon, PlusIcon, MinusIcon} from "@heroicons/react/24/outline";
...
export default function POSCheckoutLaundry() {
	const { currency, itemsCheckout, setItemsCheckout, lang, setMoney, setTotalPay, totalPay,  pajakGlobal,
		...
		setTableGlobal, cookies, customerGlobal, setCustomerGlobal, pajakGlobalJSON, setPajakGlobalJSON, totalPrice, setTotalPrice} = useContext(AppContext);
		...
		return (
			<div
				className={`action-area ${
					mobile ? "lg:hidden fixed bottom-3 inset-x-4" : "mobile:hidden w-full mx-auto"
				} flex gap-2`}
			>
				<Button
					size="lg"
					...
					{pajakGlobal == 0 ? null : (
						<ListItem className="">
							<div className="grid grid-cols-2 gap-2 p-1 w-full">
								...
								<div className="text-right">{formatThousandSeparator(((totalPrice * (1 - parseInt(diskonGlobal) / 100)) * (parseInt(pajakGlobal) / 100)))}</div>
							</div>
							<ListItemSuffix>
