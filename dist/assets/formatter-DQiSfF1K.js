const formatThousandSeparator = (number, precision = 0) => {
  let numberStr = "0";
...
const formatBackToNumber = (numberStr) => {
  const newNumberStr = numberStr.replace(/[^0-9,-]+/g, "");
  return Number(newNumberStr);
};
const formatNumber = (num) => {
  if (num >= 1e6) {
    return (num / 1e6).toFixed(2).replace(/\.00$/, "") + " juta";
  } else if (num >= 1e3) {
    return (num / 1e3).toFixed(0) + " ribu";
  } else {
    return num.toString();
  }
};
...
const formatRangeDate = (date, datenew, flag = false) => {
  var d = new Date(date), month = "" + (d.getMonth() + 1), day = "" + d.getDate(), year = d.getFullYear();
  if (month.length < 2)
    month = "0" + month;
  if (day.length < 2)
    day = "0" + day;
  var dnew = new Date(datenew), monthnew = "" + (dnew.getMonth() + 1), daynew = "" + dnew.getDate(), yearnew = dnew.getFullYear();
...
const monthConverter = (input, format = "long") => {
  if (input) {
    const monthNames = {
      long: [
        "Januari",
...
export {
  SetItemUnit as S,
  formatBackToNumber as a,
  formatSentenceCase as b,
  capitalizeWords as c,
  formatDate as d,
  formatRangeDate as e,
  formatThousandSeparator as f,
  formatNumber as g,
  formatRupiah as h,
  monthConverter as m
};
