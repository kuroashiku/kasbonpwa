...
// node_modules/.pnpm/thermal-printer-encoder@2.1.0/node_modules/thermal-printer-encoder/dist/thermal-printer-encoder.esm.js
function e(e2) {
  return e2 && e2.__esModule && Object.prototype.hasOwnProperty.call(e2, "default") ? e2.default : e2;
}
var t = { exports: {} };
var i = { html: { skipScheme: "html", lineBreakScheme: "html", whitespace: "collapse" } };
var s = /<\s*br(?:[\s/]*|\s[^>]*)>/gi;
var n = { unix: [/\n/g, "\n"], dos: [/\r\n/g, "\r\n"], mac: [/\r/g, "\r"], html: [s, "<br>"], xhtml: [s, "<br/>"] };
var r = { "ansi-color": /\x1B\[[^m]*m/g, html: /<[^>]*>/g, bbcode: /\[[^]]*\]/g };
var a = { soft: 1, hard: 1 };
var o = { collapse: 1, default: 1, line: 1, all: 1 };
var h = { all: 1, multi: 1, none: 1 };
var l = /([sm])(\d+)/;
var p = /[-/\\^$*+?.()|[\]{}]/g;
function c(e2) {
  return e2.replace(p, "\\$&");
}
var d = t.exports = function(e2, t2, s2) {
  "object" == typeof e2 && (e2 = (s2 = e2).start, t2 = s2.stop), "object" == typeof t2 && (s2 = t2, e2 = e2 || s2.start, t2 = void 0), t2 || (t2 = e2, e2 = 0), s2 || (s2 = {});
  var p2, d2, u2, g2, f2, w2, _2, b2, m2, y2, v2, k2, q2, E2, L, x, A, j, I = "soft", M = "default", R = 4, B = "all", S = "", C = "";
  if (p2 = s2.preset) for (p2 instanceof Array || (p2 = [p2]), j = 0; j < p2.length; j++) {
    if (!(x = i[p2[j]])) throw new TypeError('preset must be one of "' + Object.keys(i).join('", "') + '"');
    x.mode && (I = x.mode), x.whitespace && (M = x.whitespace), void 0 !== x.tabWidth && (R = x.tabWidth), x.skip && (d2 = x.skip), x.skipScheme && (u2 = x.skipScheme), x.lineBreak && (g2 = x.lineBreak), x.lineBreakScheme && (f2 = x.lineBreakScheme), x.respectLineBreaks && (B = x.respectLineBreaks), void 0 !== x.preservedLineIndent && (_2 = x.preservedLineIndent), void 0 !== x.wrapLineIndent && (b2 = x.wrapLineIndent), x.wrapLineIndentBase && (m2 = x.wrapLineIndentBase);
  }
  if (s2.mode) {
    if (!a[s2.mode]) throw new TypeError('mode must be one of "' + Object.keys(a).join('", "') + '"');
    I = s2.mode;
  }
  if (s2.whitespace) {
    if (!o[s2.whitespace]) throw new TypeError('whitespace must be one of "' + Object.keys(o).join('", "') + '"');
    M = s2.whitespace;
  }
  if (void 0 !== s2.tabWidth) {
    if (!(parseInt(s2.tabWidth, 10) >= 0)) throw new TypeError("tabWidth must be a non-negative integer");
    R = parseInt(s2.tabWidth, 10);
  }
  if (L = new Array(R + 1).join(" "), s2.respectLineBreaks) {
    if (!h[s2.respectLineBreaks] && !l.test(s2.respectLineBreaks)) throw new TypeError('respectLineBreaks must be one of "' + Object.keys(h).join('", "') + '", "m<num>", "s<num>"');
  ...
  if (void 0 !== s2.preservedLineIndent) {
    if (!(parseInt(s2.preservedLineIndent, 10) >= 0)) throw new TypeError("preservedLineIndent must be a non-negative integer");
    _2 = parseInt(s2.preservedLineIndent, 10);
  }
  if (_2 > 0 && (S = new Array(_2 + 1).join(" ")), void 0 !== s2.wrapLineIndent) {
    if (isNaN(parseInt(s2.wrapLineIndent, 10))) throw new TypeError("wrapLineIndent must be an integer");
    b2 = parseInt(s2.wrapLineIndent, 10);
  }
  if (s2.wrapLineIndentBase && (m2 = s2.wrapLineIndentBase), m2) {
    if (void 0 === b2) throw new TypeError("wrapLineIndent must be specified when wrapLineIndentBase is specified");
    if (m2 instanceof RegExp) E2 = m2;
    else {
      if ("string" != typeof m2) throw new TypeError("wrapLineIndentBase must be either a RegExp object or a string");
      E2 = new RegExp(c(m2));
    }
  } else if (b2 > 0) C = new Array(b2 + 1).join(" ");
  else if (b2 < 0) throw new TypeError("wrapLineIndent must be non-negative when a base is not specified");
  if (s2.skipScheme) {
    if (!r[s2.skipScheme]) throw new TypeError('skipScheme must be one of "' + Object.keys(r).join('", "') + '"');
    u2 = s2.skipScheme;
  }
  if (s2.skip && (d2 = s2.skip), d2) if (d2 instanceof RegExp) (y2 = d2).global || (A = "g", y2.ignoreCase && (A += "i"), y2.multiline && (A += "m"), y2 = new RegExp(y2.source, A));
  else {
    if ("string" != typeof d2) throw new TypeError("skip must be either a RegExp object or a string");
    y2 = new RegExp(c(d2), "g");
  }
  if (!y2 && u2 && (y2 = r[u2]), s2.lineBreakScheme) {
    if (!n[s2.lineBreakScheme]) throw new TypeError('lineBreakScheme must be one of "' + Object.keys(n).join('", "') + '"');
    f2 = s2.lineBreakScheme;
  }
  if (s2.lineBreak && (g2 = s2.lineBreak), f2 && (x = n[f2]) && (v2 = x[0], k2 = x[1]), g2) {
    if (g2 instanceof Array && (1 === g2.length ? g2 = g2[0] : g2.length >= 2 && (g2[0] instanceof RegExp ? (v2 = g2[0], "string" == typeof g2[1] && (k2 = g2[1])) : g2[1] instanceof RegExp ? (v2 = g2[1], "string" == typeof g2[0] && (k2 = g2[0])) : "string" == typeof g2[0] && "string" == typeof g2[1] ? (v2 = new RegExp(c(g2[0]), "g"), k2 = g2[1]) : g2 = g2[0])), "string" == typeof g2) k2 = g2, v2 || (v2 = new RegExp(c(g2), "g"));
    else if (g2 instanceof RegExp) v2 = g2;
    else if (!(g2 instanceof Array)) throw new TypeError("lineBreak must be a RegExp object, a string, or an array consisted of a RegExp object and a string");
  }
  v2 || (v2 = /\n/g, k2 = "\n"), A = "g", v2.ignoreCase && (A += "i"), v2.multiline && (A += "m"), q2 = new RegExp("\\s*(?:" + v2.source + ")(?:" + v2.source + "|\\s)*", A), v2.global || (v2 = new RegExp(v2.source, A));
  var z = "hard" === I ? /\b/ : /(\S+\s+)/, T = new Array(e2 + 1).join(" "), U = "default" === M || "collapse" === M, W = "collapse" === M, N = "line" === M, P = "all" === M, H = /\t/g, F = /  +/g, G = /^\s+/, K = /\s+$/, $ = /\S/, Q = /\s/, D = t2 - e2;
  return function(i2) {
    var s3;
    if (i2 = i2.toString().replace(H, L), !k2) {
      if (v2.lastIndex = 0, !(s3 = v2.exec(i2))) throw new TypeError("Line break string for the output not specified");
      k2 = s3[0];
    }
    var n2, r2, a2, o2, h2, l2, p3, c2, d3, u3 = 0;
    for (n2 = [], q2.lastIndex = 0, s3 = q2.exec(i2); s3; ) {
      if (n2.push(i2.substring(u3, s3.index)), "none" !== B) {
        for (a2 = [], o2 = 0, v2.lastIndex = 0, r2 = v2.exec(s3[0]); r2; ) a2.push(s3[0].substring(o2, r2.index)), o2 = r2.index + r2[0].length, r2 = v2.exec(s3[0]);
        a2.push(s3[0].substring(o2)), n2.push({ type: "break", breaks: a2 });
      } else h2 = W ? " " : s3[0].replace(v2, ""), n2.push({ type: "break", remaining: h2 });
      u3 = s3.index + s3[0].length, s3 = q2.exec(i2);
    }
    if (n2.push(i2.substring(u3)), y2) for (d3 = [], l2 = 0; l2 < n2.length; l2++) {
      var g3 = n2[l2];
      if ("string" != typeof g3) d3.push(g3);
      else {
        for (u3 = 0, y2.lastIndex = 0, s3 = y2.exec(g3); s3; ) d3.push(g3.substring(u3, s3.index)), d3.push({ type: "skip", value: s3[0] }), u3 = s3.index + s3[0].length, s3 = y2.exec(g3);
        d3.push(g3.substring(u3));
      }
    }
    else d3 = n2;
    var f3 = [];
    for (l2 = 0; l2 < d3.length; l2++) {
      var _3 = d3[l2];
      if ("string" != typeof _3) f3.push(_3);
      else {
        W && (_3 = _3.replace(F, " "));
        var m3 = _3.split(z), x2 = [];
        for (p3 = 0; p3 < m3.length; p3++) {
          var A2 = m3[p3];
          if ("hard" === I) for (c2 = 0; c2 < A2.length; c2 += D) x2.push(A2.slice(c2, c2 + D));
          else x2.push(A2);
        }
        f3 = f3.concat(x2);
      }
    ...
    function Z(i3) {
      ...
      if (P) R2 > t2 && (V = V || t2, r3 = a3.length - (R2 - V), O2[M2] = a3.substring(0, r3)), V = 0;
      else {
        for (s4 = a3.length - 1; s4 >= e2 && " " === a3[s4]; ) s4--;
        for (; s4 >= e2 && Q.test(a3[s4]); ) s4--;
      ...
      if (J && (J = false, E2 && (s4 = O2[M2].substring(e2).search(E2), X = s4 >= 0 && s4 + b2 > 0 ? new Array(s4 + b2 + 1).join(" ") : "")), r3) {
        for (; r3 + D < a3.length; ) P ? (n3 = a3.substring(r3, r3 + D), O2.push(T + X + n3)) : O2.push(T + X), r3 += D, M2++;
        if (!i3) return n3 = a3.substring(r3), X + n3;
    ...
    for (l2 = 0; l2 < f3.length; l2++) {
      var ee = f3[l2];
      if ("" !== ee) if ("string" == typeof ee) {
        for (var te; ; ) {
          if (te = void 0, R2 + ee.length > t2 && R2 + (te = ee.replace(K, "")).length > t2 && "" !== te && R2 > e2) {
            if (j2 = Z(false), O2.push(T + X), M2++, R2 = e2 + X.length, j2) {
              O2[M2] += j2, R2 += j2.length, Y = true;
              continue;
            }
            !U && (!N || J && Y) || (ee = ee.replace(G, "")), Y = false;
          } else Y && (U || N && (!J || !Y) ? "" !== (ee = ee.replace(G, "")) && (Y = false) : $.test(ee) && (Y = false));
          break;
        }
        P && te && R2 + te.length > t2 && (V = R2 + te.length), O2[M2] += ee, R2 += ee.length;
      } else if ("break" === ee.type) if ("none" !== B) {
        var ie = ee.breaks, se = ie.length - 1;
        if ("s" === B) {
          for (p3 = 0; p3 < se; p3++) ie[p3 + 1].length < w2 ? ie[p3 + 1] = W ? " " : ie[p3] + ie[p3 + 1] : (P && (O2[M2] += ie[p3], R2 += ie[p3].length), Z(true), O2.push(T + S), M2++, R2 = e2 + S.length, J = Y = true);
          (!Y || P || N && J) && ((W || !Y && "" === ie[se]) && (ie[se] = " "), O2[M2] += ie[se], R2 += ie[se].length);
        } else if ("m" === B && se < w2) (!Y || P || N && J) && (W ? ee = " " : (ee = ie.join(""), Y || "" !== ee || (ee = " ")), O2[M2] += ee, R2 += ee.length);
        else if (U) {
          for (Z(true), p3 = 0; p3 < se; p3++) O2.push(T + S), M2++;
          R2 = e2 + S.length, J = Y = true;
        } else for ((P || J && Y) && (O2[M2] += ie[0], R2 += ie[0].length), p3 = 0; p3 < se; p3++) Z(true), O2.push(T + S + ie[p3 + 1]), M2++, R2 = e2 + S.length + ie[p3 + 1].length, J = Y = true;
      } else (!Y || P || N && J) && (ee = ee.remaining, (W || !Y && "" === ee) && (ee = " "), O2[M2] += ee, R2 += ee.length);
      else "skip" === ee.type && (R2 > t2 && (j2 = Z(false), O2.push(T + X), M2++, R2 = e2 + X.length, j2 && (O2[M2] += j2, R2 += j2.length), Y = true), O2[M2] += ee.value);
    }
    return Z(true), O2.join(k2);
  };
};
d.soft = d, d.hard = function() {
  var e2 = [].slice.call(arguments), t2 = e2.length - 1;
  return "object" == typeof e2[t2] ? e2[t2].mode = "hard" : e2.push({ mode: "hard" }), d.apply(null, e2);
}, d.wrap = function(e2) {
  var t2 = [].slice.call(arguments);
  return t2.shift(), d.apply(null, t2)(e2);
};
var u = e(t.exports);
...
var f = e(new class {
  ...
  floydsteinberg(e2) {
    ...
    for (let t3 = 0, s2 = 0; s2 < e2.data.length; t3++, s2 += 4) i2[t3] = 0.299 * e2.data[s2] + 0.587 * e2.data[s2 + 1] + 0.114 * e2.data[s2 + 2];
...
var m = class {
  ...
  static autoEncode(e2, t2) {
    ...
    for (let r2 = 0; r2 < e2.length; r2++) {
      ...
      if (!o2) for (let i3 = 0; i3 < t2.length; i3++) {
...
var v = class _v {
  ...
  _encode(e2) {
    if ("auto" != this._codepage) return m.encode(e2, this._codepage);
    let t2;
    ...
    let s2 = 0;
    for (let e3 = 0; e3 < i2.length; e3++) s2 += 3 + i2[e3].bytes.byteLength;
    const n2 = new Uint8Array(s2);
    let r2 = 0;
    for (let e3 = 0; e3 < i2.length; e3++) n2.set([27, 116, t2[i2[e3].codepage]], r2), n2.set(i2[e3].bytes, r2 + 3), r2 += 3 + i2[e3].bytes.byteLength, this._state.codepage = t2[i2[e3].codepage];
    return n2;
  }
  _queue(e2) {
  ...
  codepage(e2) {
    if ("auto" === e2) return this._codepage = e2, this;
    if (!m.supports(e2)) throw new Error("Unknown codepage");
    let t2;
    if (t2 = "string" == typeof this._options.codepageMapping ? y[this._options.codepageMapping] : this._options.codepageMapping, void 0 === t2[e2]) throw new Error("Codepage not supported by printer");
  ...
  align(e2) {
    ...
    if (!(e2 in t2)) throw new Error("Unknown alignment");
    ...
    for (let i2 = 0; i2 < t2.length; i2++) {
      ...
      for (let r2 = 0; r2 < e2.length; r2++) {
        ...
        if ("string" == typeof t2[i2][r2]) {
          ...
          for (let t3 = 0; t3 < s3.length; t3++) "right" == e2[r2].align ? a2[t3] = this._encode(s3[t3].padStart(e2[r2].width)) : a2[t3] = this._encode(s3[t3].padEnd(e2[r2].width));
        }
        if ("function" == typeof t2[i2][r2]) {
          const s3 = new _v(Object.assign({}, this._options, { width: e2[r2].width, embedded: true }));
          s3._codepage = this._codepage, s3.align(e2[r2].align), t2[i2][r2](s3);
          const n3 = s3.encode();
          let o2 = [];
          for (let e3 = 0; e3 < n3.byteLength; e3++) e3 < n3.byteLength - 1 && 10 === n3[e3] && 13 === n3[e3 + 1] ? (a2.push(o2), o2 = [], e3++) : o2.push(n3[e3]);
          o2.length && a2.push(o2);
        }
        n2 = Math.max(n2, a2.length), s2[r2] = a2;
      }
      for (let t3 = 0; t3 < e2.length; t3++) if (s2[t3].length < n2) for (let i3 = s2[t3].length; i3 < n2; i3++) {
        let i4 = "top";
      ...
      for (let t3 = 0; t3 < n2; t3++) {
        for (let i3 = 0; i3 < e2.length; i3++) void 0 !== e2[i3].marginLeft && this.raw(new Array(e2[i3].marginLeft).fill(32)), this.raw(s2[i3][t3]), void 0 !== e2[i3].marginRight && this.raw(new Array(e2[i3].marginRight).fill(32));
        this.newline();
      }
    }
    return this;
  ...
  box(e2, t2) {
    ...
    if ("string" == typeof t2) {
      ...
      for (let t3 = 0; t3 < i3.length; t3++) "right" == e2.align ? s2[t3] = this._encode(i3[t3].padStart(e2.width - 2 - e2.paddingLeft - e2.paddingRight)) : s2[t3] = this._encode(i3[t3].padEnd(e2.width - 2 - e2.paddingLeft - e2.paddingRight));
    }
    if ("function" == typeof t2) {
      const i3 = new _v(Object.assign({}, this._options, { width: e2.width - 2 - e2.paddingLeft - e2.paddingRight, embedded: true }));
      i3._codepage = this._codepage, i3.align(e2.align), t2(i3);
      const n2 = i3.encode();
      let r2 = [];
      for (let e3 = 0; e3 < n2.byteLength; e3++) e3 < n2.byteLength - 1 && 10 === n2[e3] && 13 === n2[e3 + 1] ? (s2.push(r2), r2 = [], e3++) : r2.push(n2[e3]);
      r2.length && s2.push(r2);
    }
    for (let t3 = 0; t3 < s2.length; t3++) this._queue([new Array(e2.marginLeft).fill(32), i2[5], new Array(e2.paddingLeft).fill(32)]), this._queue([s2[t3]]), this._restoreState(), this._queue([27, 116, this._getCodepageIdentifier("cp437")]), this._queue([new Array(e2.paddingRight).fill(32), i2[5], new Array(e2.marginRight).fill(32)]), this.newline();
  ...
  barcode(e2, t2, i2) {
    ...
    if (!(t2 in s2)) throw new Error("Symbology not supported by printer");
    {
  ...
  qrcode(e2, t2, i2, s2) {
    ...
    this._queue([10]);
    const n2 = { 1: 49, 2: 50 };
    if (void 0 === t2 && (t2 = 2), !(t2 in n2)) throw new Error("Model must be 1 or 2");
    if (this._queue([29, 40, 107, 4, 0, 49, 65, n2[t2], 0]), void 0 === i2 && (i2 = 6), "number" != typeof i2) throw new Error("Size must be a number");
    if (i2 < 1 || i2 > 8) throw new Error("Size must be between 1 and 8");
    this._queue([29, 40, 107, 3, 0, 49, 67, i2]);
    const r2 = { l: 48, m: 49, q: 50, h: 51 };
    if (void 0 === s2 && (s2 = "m"), !(s2 in r2)) throw new Error("Error level must be l, m, q or h");
  ...
  image(e2, t2, i2, s2, n2) {
    ...
    if (t2 % 8 != 0) throw new Error("Width must be a multiple of 8");
    if (i2 % 8 != 0) throw new Error("Height must be a multiple of 8");
    ...
    const o2 = (e3, s3) => e3 < t2 && s3 < i2 ? a2.data[4 * (t2 * s3 + e3)] > 0 ? 0 : 1 : 0, h2 = (e3, t3) => {
      ...
      for (let s3 = 0; s3 < t3; s3++) for (let t4 = 0; t4 < e3; t4 += 8) for (let n3 = 0; n3 < 8; n3++) i3[s3 * (e3 >> 3) + (t4 >> 3)] |= o2(t4 + n3, s3) << 7 - n3;
      return i3;
    };
    return 0 != this._cursor && this.newline(), "column" == this._options.imageMode && (this._queue([27, 51, 36]), ((e3, t3) => {
      ...
      for (let s3 = 0; s3 < Math.ceil(t3 / 24); s3++) {
        ...
        for (let i4 = 0; i4 < e3; i4++) for (let e4 = 0; e4 < 3; e4++) for (let n3 = 0; n3 < 8; n3++) t4[3 * i4 + e4] |= o2(i4, 24 * s3 + n3 + 8 * e4) << 7 - n3;
        i3.push(t4);
      }
      return i3;
    })(t2, i2).forEach((e3) => {
...
var k = { star: { cp437: 1, cp858: 4, cp852: 5, cp860: 6, cp861: 7, cp863: 8, cp865: 9, cp866: 10, cp855: 11, cp857: 12, cp862: 13, cp864: 14, cp737: 15, cp869: 17, cp874: 20, windows1252: 32, windows1250: 33, windows1251: 34 } };
var q = class _q {
  constructor(e2) {
    this._reset(e2 || {});
  }
  _reset(e2) {
    e2 && (this._options = Object.assign({ width: null, embedded: false, wordWrap: true, language: "star-prnt", codepageMapping: "star", codepageCandidates: ["cp437", "cp858", "cp860", "cp861", "cp863", "cp865", "cp852", "cp857", "cp855", "cp866", "cp869"] }, e2), void 0 === this._options.autoFlush && (this._options.autoFlush = !this._options.embedded && "star-prnt" == this._options.language)), this._embedded = this._options.width && this._options.embedded, this._buffer = [], this._queued = [], this._cursor = 0, this._codepage = "ascii", this._state = { codepage: 0, align: "left", bold: false, italic: false, underline: false, invert: false, width: 1, height: 1 };
  }
  _encode(e2) {
    if ("auto" != this._codepage) return m.encode(e2, this._codepage);
    let t2;
    ...
    const i2 = m.autoEncode(e2, this._options.codepageCandidates);
    let s2 = 0;
    for (let e3 = 0; e3 < i2.length; e3++) s2 += 4 + i2[e3].bytes.byteLength;
    const n2 = new Uint8Array(s2);
    let r2 = 0;
    for (let e3 = 0; e3 < i2.length; e3++) n2.set([27, 29, 116, t2[i2[e3].codepage]], r2), n2.set(i2[e3].bytes, r2 + 4), r2 += 4 + i2[e3].bytes.byteLength, this._state.codepage = t2[i2[e3].codepage];
    return n2;
  }
  _queue(e2) {
    e2.forEach((e3) => this._queued.push(e3));
  ...
  codepage(e2) {
    ...
    if (!m.supports(e2)) throw new Error("Unknown codepage");
    let t2;
    if (t2 = "string" == typeof this._options.codepageMapping ? k[this._options.codepageMapping] : this._options.codepageMapping, void 0 === t2[e2]) throw new Error("Codepage not supported by printer");
  ...
  width(e2) {
    if (void 0 === e2 && (e2 = 1), "number" != typeof e2) throw new Error("Width must be a number");
    if (e2 < 1 || e2 > 6) throw new Error("Width must be between 1 and 6");
    return this._state.width = e2, this._queue([27, 105, this._state.height - 1, this._state.width - 1]), this;
  }
  height(e2) {
    if (void 0 === e2 && (e2 = 1), "number" != typeof e2) throw new Error("Height must be a number");
    if (e2 < 1 || e2 > 6) throw new Error("Height must be between 1 and 6");
  ...
  table(e2, t2) {
    ...
    for (let i2 = 0; i2 < t2.length; i2++) {
      ...
      for (let r2 = 0; r2 < e2.length; r2++) {
        ...
        if ("function" == typeof t2[i2][r2]) {
          const s3 = new _q(Object.assign({}, this._options, { width: e2[r2].width, embedded: true }));
          s3._codepage = this._codepage, s3.align(e2[r2].align), t2[i2][r2](s3);
          const n3 = s3.encode();
          let o2 = [];
          for (let e3 = 0; e3 < n3.byteLength; e3++) e3 < n3.byteLength - 1 && 10 === n3[e3] && 13 === n3[e3 + 1] ? (a2.push(o2), o2 = [], e3++) : o2.push(n3[e3]);
  ...
  box(e2, t2) {
    ...
    if ("function" == typeof t2) {
      const i3 = new _q(Object.assign({}, this._options, { width: e2.width - 2 - e2.paddingLeft - e2.paddingRight, embedded: true }));
      i3._codepage = this._codepage, i3.align(e2.align), t2(i3);
      const n2 = i3.encode();
      let r2 = [];
      for (let e3 = 0; e3 < n2.byteLength; e3++) e3 < n2.byteLength - 1 && 10 === n2[e3] && 13 === n2[e3 + 1] ? (s2.push(r2), r2 = [], e3++) : r2.push(n2[e3]);
    ...
    for (let t3 = 0; t3 < s2.length; t3++) this._queue([new Array(e2.marginLeft).fill(32), i2[5], new Array(e2.paddingLeft).fill(32)]), this._queue([s2[t3]]), this._restoreState(), this._queue([27, 29, 116, this._getCodepageIdentifier("cp437")]), this._queue([new Array(e2.paddingRight).fill(32), i2[5], new Array(e2.marginRight).fill(32)]), this.newline();
    return this._queue([new Array(e2.marginLeft).fill(32), i2[2], new Array(e2.width - 2).fill(i2[4]), i2[3], new Array(e2.marginRight).fill(32)]), this._restoreState(), this.newline(), this;
  }
  barcode(e2, t2, i2) {
    if (this._embedded) throw new Error("Barcodes are not supported in table cells or boxes");
  ...
  qrcode(e2, t2, i2, s2) {
    ...
    const n2 = { 1: 1, 2: 2 };
    if (void 0 === t2 && (t2 = 2), !(t2 in n2)) throw new Error("Model must be 1 or 2");
    if (this._queue([27, 29, 121, 83, 48, n2[t2]]), void 0 === i2 && (i2 = 6), "number" != typeof i2) throw new Error("Size must be a number");
    if (i2 < 1 || i2 > 8) throw new Error("Size must be between 1 and 8");
  ...
  image(e2, t2, i2, s2, n2) {
    if (this._embedded) throw new Error("Images are not supported in table cells or boxes");
    if (t2 % 8 != 0) throw new Error("Width must be a multiple of 8");
    if (i2 % 24 != 0) throw new Error("Height must be a multiple of 24");
    void 0 === s2 && (s2 = "threshold"), void 0 === n2 && (n2 = 128);
    const r2 = g(t2, i2).getContext("2d");
    r2.drawImage(e2, 0, 0, t2, i2);
    let a2 = r2.getImageData(0, 0, t2, i2);
  ...
  flush() {
    return this._queue([27, 29, 80, 48, 27, 29, 80, 49]), this;
  }
  raw(e2) {
    return this._queue(e2), this;
  }
  encode() {
    this._options.autoFlush && this.flush(), this._flush();
    let e2 = 0;
    this._buffer.forEach((t3) => {
      "number" == typeof t3 ? e2++ : e2 += t3.length;
    });
...
var E = class {
  constructor(e2) {
    const t2 = { "esc-pos": v, "star-prnt": q, "star-line": q };
    if (void 0 === e2 || void 0 === e2.language) throw new Error("You need to specify the language of the thermal printer");
    if (void 0 === t2[e2.language]) throw new Error("Language not supported by this library");
...
//# sourceMappingURL=thermal-printer-encoder.js.map
